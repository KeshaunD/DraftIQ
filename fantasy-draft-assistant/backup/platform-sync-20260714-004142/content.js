(function () {
      ...
      <section class="dc-drawer dc-drawer-hidden" id="dc-settings-drawer" aria-label="League settings">
        ...
        <form class="dc-settings-form" id="dc-settings-form">
          ...
          <div class="dc-settings-note">
            DraftIQ uses these rules for roster needs, snake-draft timing, and recommendations.
          </div>
  ...
  function saveLeagueSettings(event) {
    ...
    chrome.storage.local.set(
      { [STORAGE_KEY_LEAGUE_SETTINGS]: leagueSettings },
      () => {
  ...
  function renderDiagnostics() {
    ...
    const warning = !diagnostics
      ...
      : !diagnostics.supported
        ? `${platform} is visible, but automatic pick detection is currently tuned for Yahoo.`
        : !healthy
    ...
    body.innerHTML = `
      ...
      <div class="dc-diagnostics-tip">
        Tip: keep the Yahoo draft tab open. DraftIQ can scan while you view other tabs, but Yahoo must remain loaded.
      </div>
    `;
  }
    ...
    if (!report.players.length) {
      body.innerHTML = `
        <div class="dc-empty-state">
          ...
          <span>DraftIQ will build this report as Yahoo identifies players on your team.</span>
        </div>
      `;
      return;
    }
