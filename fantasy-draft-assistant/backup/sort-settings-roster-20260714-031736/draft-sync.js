(function () {
  ...
  async function loadLeagueSettings() {
    ...
    const saved = current[STORAGE_KEY_LEAGUE_SETTINGS] || {};
  ...
  function isConfiguredYahooPick(overallPick) {
    ...
    const round = Math.ceil(pick / configuredTeamCount);
    const positionInRound =
      ((pick - 1) % configuredTeamCount) + 1;
    const draftSlot =
      round % 2 === 1
        ? positionInRound
        : configuredTeamCount - positionInRound + 1;
