(function () {
  ...
  root.innerHTML = `
    ...
    <div class="dc-window" id="dc-window">
      ...
      <section class="dc-drawer dc-drawer-hidden" id="dc-settings-drawer" aria-label="League settings">
        ...
        <section class="dc-yahoo-sync-card" id="dc-yahoo-sync-card" aria-label="Yahoo league connection">
          <div class="dc-yahoo-sync-header">
            <div>
              <div class="dc-yahoo-sync-kicker">Read-only league import</div>
              <div class="dc-yahoo-sync-title">Yahoo League</div>
            </div>
            <span class="dc-yahoo-sync-pill" id="dc-yahoo-sync-pill">Checking</span>
          </div>
          <p class="dc-yahoo-sync-copy" id="dc-yahoo-sync-copy">
            Checking the private Yahoo connection...
          </p>
          <select class="dc-yahoo-league-select" id="dc-yahoo-league-select" aria-label="Yahoo league" disabled>
            <option value="">No league loaded</option>
          </select>
          <div class="dc-yahoo-sync-actions">
            <button class="dc-secondary-button" id="dc-yahoo-connect" type="button">Connect Yahoo</button>
            <button class="dc-primary-button dc-yahoo-sync-button" id="dc-yahoo-sync" type="button" disabled>Sync league</button>
          </div>
          <button class="dc-yahoo-check-button" id="dc-yahoo-check" type="button">Check connection</button>
        </section>
  ...
  let yahooLeagueSnapshot = null;
  let yahooSelectedLeague = "";
  let yahooLeagues = [];
  let yahooHelperStatus = null;
  let yahooSyncBusy = false;
  ...
  function renderYahooConnection() {
    const pill = shadow.getElementById("dc-yahoo-sync-pill");
    const copy = shadow.getElementById("dc-yahoo-sync-copy");
    const select = shadow.getElementById("dc-yahoo-league-select");
    const connectButton = shadow.getElementById("dc-yahoo-connect");
    const syncButton = shadow.getElementById("dc-yahoo-sync");
    const checkButton = shadow.getElementById("dc-yahoo-check");

    if (!pill || !copy || !select) return;

    const online = Boolean(yahooHelperStatus?.ok);
    const configured = Boolean(yahooHelperStatus?.configured);
    const connected = Boolean(yahooHelperStatus?.connected);
    const snapshotLeague = yahooLeagueSnapshot?.league;
    const lastSynced = formatUpdatedAt(yahooLeagueSnapshot?.syncedAt);

    pill.classList.remove(
      "dc-yahoo-sync-pill-live",
      "dc-yahoo-sync-pill-warning"
    );

    if (connected) {
      pill.textContent = "Connected";
      pill.classList.add("dc-yahoo-sync-pill-live");
    } else if (snapshotLeague) {
      pill.textContent = "Saved";
    } else {
      pill.textContent = online ? "Setup" : "Offline";
      pill.classList.add("dc-yahoo-sync-pill-warning");
    }

    if (yahooSyncBusy) {
      copy.textContent = "Contacting Yahoo and importing league data...";
    } else if (!online && snapshotLeague) {
      copy.textContent = `${snapshotLeague.name} was last synced${
        lastSynced ? ` ${lastSynced}` : ""
      }. Start the local helper to refresh it.`;
    } else if (!online) {
      copy.textContent =
        yahooHelperStatus?.error ||
        "Start yahoo-sync-helper/start.ps1 to connect your private Yahoo leagues.";
    } else if (!configured) {
      copy.textContent =
        "The helper is running. Add your Yahoo Client ID and Client Secret to its .env file, then restart it.";
    } else if (!connected) {
      copy.textContent =
        "The helper is ready. Connect Yahoo once, then return here and check the connection.";
    } else if (!yahooLeagues.length && !snapshotLeague) {
      copy.textContent =
        yahooHelperStatus?.error || "Yahoo is connected, but no football leagues were returned.";
    } else if (snapshotLeague) {
      copy.textContent = `${snapshotLeague.name} is linked${
        lastSynced ? ` · synced ${lastSynced}` : ""
      }. Sync again whenever rosters or league rules change.`;
    } else {
      copy.textContent = "Choose the Yahoo football league DraftIQ should use.";
    }

    const leaguesToRender = yahooLeagues.length
      ? yahooLeagues
      : snapshotLeague
        ? [snapshotLeague]
        : [];

    select.innerHTML = leaguesToRender.length
      ? leaguesToRender
          .map(
            (league) =>
              `<option value="${escapeMarkup(league.key)}">${escapeMarkup(
                league.name
              )}${league.teamCount ? ` · ${league.teamCount} teams` : ""}</option>`
          )
          .join("")
      : '<option value="">No league loaded</option>';

    const preferredLeague =
      yahooSelectedLeague || snapshotLeague?.key || leaguesToRender[0]?.key || "";

    if (leaguesToRender.some((league) => league.key === preferredLeague)) {
      select.value = preferredLeague;
    }

    yahooSelectedLeague = select.value;
    select.disabled = yahooSyncBusy || !connected || !yahooLeagues.length;
    connectButton.disabled = yahooSyncBusy;
    connectButton.textContent = connected ? "Manage Yahoo" : "Connect Yahoo";
    syncButton.disabled = yahooSyncBusy || !connected || !select.value;
    syncButton.textContent = yahooSyncBusy ? "Syncing..." : "Sync league";
    checkButton.disabled = yahooSyncBusy;
  }

  async function refreshYahooConnection() {
    if (yahooSyncBusy) return;

    yahooSyncBusy = true;
    renderYahooConnection();

    const status = await sendRuntimeMessage({ type: "DRAFTIQ_YAHOO_STATUS" });
    yahooHelperStatus = status;

    if (status.ok && status.connected) {
      const leaguesResult = await sendRuntimeMessage({
        type: "DRAFTIQ_YAHOO_LEAGUES",
      });

      if (leaguesResult.ok) {
        yahooLeagues = Array.isArray(leaguesResult.leagues)
          ? leaguesResult.leagues
          : [];
      } else {
        yahooHelperStatus = { ...status, error: leaguesResult.error };
      }
    }
  ...
  async function openYahooConnection() {
    const response = await sendRuntimeMessage({ type: "DRAFTIQ_YAHOO_CONNECT" });

    if (!response.ok) {
      yahooHelperStatus = { ok: false, error: response.error };
      renderYahooConnection();
      return;
    }

    showFooterStatus(
      "Yahoo connection page opened. Finish there, then check the connection.",
      "success",
      6000
    );
  }

  async function syncSelectedYahooLeague() {
    const select = shadow.getElementById("dc-yahoo-league-select");
    const leagueKey = select?.value;

    if (!leagueKey || yahooSyncBusy) return;

    yahooSelectedLeague = leagueKey;
    yahooSyncBusy = true;
    renderYahooConnection();
    ...
    if (!result.ok) {
      ...
      showFooterStatus(`Yahoo sync failed: ${result.error}`, "error", 7000);
      renderYahooConnection();
      return;
    }

    yahooLeagueSnapshot = result.snapshot;
    renderYahooConnection();
    showFooterStatus(
      `${result.snapshot.league?.name || "Yahoo league"} synced · ${
        result.snapshot.teams?.length || 0
      } teams imported.`,
      "success",
      6000
    );
  ...
  function loadState() {
    chrome.storage.local.get(
      ...
      (res) => {
        ...
        yahooLeagueSnapshot = res[STORAGE_KEY_YAHOO_LEAGUE] || null;
        yahooSelectedLeague =
          res[STORAGE_KEY_YAHOO_SELECTED_LEAGUE] ||
          yahooLeagueSnapshot?.league?.key ||
          "";
  ...
  shadow
    .getElementById("dc-yahoo-connect")
    .addEventListener("click", openYahooConnection);

  shadow
    .getElementById("dc-yahoo-sync")
    .addEventListener("click", syncSelectedYahooLeague);

  shadow
    .getElementById("dc-yahoo-check")
    .addEventListener("click", refreshYahooConnection);

  shadow
    .getElementById("dc-yahoo-league-select")
    .addEventListener("change", (event) => {
      yahooSelectedLeague = event.target.value;
      renderYahooConnection();
    });
  ...
  chrome.storage.onChanged.addListener(
    (changes, area) => {
      ...
      if (changes[STORAGE_KEY_YAHOO_LEAGUE]) {
        yahooLeagueSnapshot =
          changes[STORAGE_KEY_YAHOO_LEAGUE].newValue || null;
        renderYahooConnection();
      }

      if (changes[STORAGE_KEY_YAHOO_SELECTED_LEAGUE]) {
        yahooSelectedLeague =
          changes[STORAGE_KEY_YAHOO_SELECTED_LEAGUE].newValue || "";
        renderYahooConnection();
      }
